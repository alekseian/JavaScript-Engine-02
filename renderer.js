...
export class Renderer {
    ...
    drawFrame(objects, fillCol, bordCol) {
        ...
        for (let i = 0; i<this.renderedNextFrame.length; i++) {
            this.renderedNextFrame[i].draw(this.ctx, bordCol); //draw each time from the list 
        }     
        this.renderedNextFrame = [];  //clear the array, basically means we only draw one time
